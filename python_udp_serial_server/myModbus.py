...
#
#   File is modified to work with Modbus message extensions
#   Glenn Edgar Onyx Engineering, Incl
#
...
class Instrument():
    ...
    def __init__(self, port, slaveaddress, mode=MODE_RTU):
        ...
        """
        self.serial = _SERIALPORTS[port] = serial.Serial(port=port, baudrate=BAUDRATE, parity=PARITY, bytesize=BYTESIZE, stopbits=STOPBITS, timeout=TIMEOUT)
        #if self.serial.port is None: # not needed as serial port is automatically opened
        #        self.serial.open()
    ...
    def read_eeprom_registers(self, registeraddress, numberOfRegisters, functioncode=30):
        ...
        """
        _checkFunctioncode(functioncode, [30])
        _checkInt(numberOfRegisters, minvalue=1, description='number of registers')
    ...
    def read_fifo(self, registeraddress, numberOfRegisters, functioncode=33):
        ...
        """
        _checkFunctioncode(functioncode, [33])
        _checkInt(numberOfRegisters, minvalue=1, description='number of registers')
        temp = self._genericCommand(functioncode, registeraddress, \
            numberOfRegisters=numberOfRegisters, payloadformat='registers')
        length = ord(temp[:1])
    ...
    def _genericCommand(self, functioncode, registeraddress, value=None, \
        ...
        if functioncode in [16,31,40] and payloadformat in [PAYLOADFORMAT_REGISTER, PAYLOADFORMAT_FLOAT, PAYLOADFORMAT_LONG]:
            _checkNumerical(value, description='input value')
        ...
        if functioncode == 6 and payloadformat == PAYLOADFORMAT_REGISTER:
            _checkNumerical(value, description='input value')
                    ...
                    # Value for string
        if functioncode ==[16,31,40] and payloadformat == PAYLOADFORMAT_STRING:
            ...
            # Note: The string might be padded later, so the length might be shorter than numberOfRegisterBytes.
                    ...
                    # Value for registers
        if functioncode in [16,31,40] and payloadformat == PAYLOADFORMAT_REGISTERS:
        ...
        if functioncode in [1, 2, 3, 4,30]:
            _checkResponseByteCount(payloadFromSlave)  # response byte count

        if functioncode in [5, 6, 15, 16,40]:
